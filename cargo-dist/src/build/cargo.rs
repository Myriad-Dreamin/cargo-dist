...
pub fn build_cargo_target(
    ...
    if let Some(messages_read) = messages_read {
        let pkgs = expected.packages.keys().collect::<Vec<_>>();
        eprintln!("cross for loongarch64: expected packages: {pkgs:?}");

        for message in messages_read {
            eprintln!("cross for loongarch64: {message:?}");
        }
        ...
        if is_loongarch64 {
            ...
            for file in &files {
                eprintln!("cross for loongarch64: {file:?}");
            }
